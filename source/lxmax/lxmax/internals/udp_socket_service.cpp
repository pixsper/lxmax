...
namespace lxmax
{
	udp_socket_service::udp_socket_service(asio::io_service& service,
	...
	bool udp_socket_service::send(const uint8_t* buffer, size_t buffer_length, const udp::endpoint& destination)
	{
		const size_t bytes_sent = _socket.send_to(asio::buffer(buffer, buffer_length), destination);
		return bytes_sent == buffer_length;
	}

	void udp_socket_service::join_multicast_group(const asio::ip::address_v4& multicast_address)
	{
		_socket.set_option(asio::ip::multicast::join_group(multicast_address));
	}

	void udp_socket_service::leave_multicast_group(const asio::ip::address_v4& multicast_address)
	{
		_socket.set_option(asio::ip::multicast::leave_group(multicast_address));
	}

	void udp_socket_service::start_receive()
	{
